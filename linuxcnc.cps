...
description = "PrintNC XYYZA - 4th axis v2";
vendor = "LinuxCNC";
vendorUrl = "http://www.linuxcnc.org";
longDescription = "PrintNC XYYZA - milling post for LinuxCNC (EMC2).";
...
properties = {
  ...
  useM600: {
    title      : "Use M600",
    description: "Use M600 for tool change.",
    group      : "preferences",
    type       : "boolean",
    value      : true,
    scope      : "post" 
...
var ijkFormat = createFormat({decimals:(unit == MM ? 3 : 4), forceDecimal:true});
...
//probing block status information
var probing_status = 0;
var pointNumber =0;
...
function onCycleEnd() {
  ...
  else if (!cycleExpanded || cycleType!="inspect") {
...
function onCommand(command) {
  ...
  case COMMAND_PROBE_ON:
    writeComment("LOGOPEN, inspect"+getGlobalParameter("document-id","")+".txt");
    ...
    writeComment("LOG, \n");
    writeComment("LOG, TOOLPATHID " + getParameter("autodeskcam:operation-id", ""));
    writeComment("LOG, TOOLPATH " + getParameter("operation-comment", ""));
    writteG331();
    writteG330();
    return;
  case COMMAND_PROBE_OFF:
    writeComment("LOG, END")
...
function writeRetract() {
  ...
  */
  ...
  /* LDC: Disabled going home for X and Y, to avoid crushing into 4th axis */
